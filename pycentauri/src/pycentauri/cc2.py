...
class CC2Printer(Printer):
    ...
    async def jog_axis(
        ...
        *,
        ...
        """Jog a single axis by a relative distance (mm) using CC2 method 1027.

        ``axis`` is ``X``, ``Y``, or ``Z`` (case-insensitive). ``distance``
        may be negative. The printer must be homed on the requested axis;
        method 1027 returns ``error_code: 1003`` otherwise.
        """
        self._require_control("jog_axis")
        axis_clean = axis.strip().lower()
        if axis_clean not in ("x", "y", "z"):
            raise ValueError(f"axis must be X, Y, or Z; got {axis!r}")
        params: dict[str, Any] = {"axis": axis_clean, "step": float(distance)}
        if feedrate is not None:
            params["feedrate"] = int(feedrate)
        result = await self._cc2_request(1027, params)
        return self._wrap_result(1027, result)

    async def home(self, axes: str = "ALL") -> sdcp.ParsedMessage:
        """Home axes using CC2 method 1026.

        The confirmed CC2 payload homes all axes: ``{}``. ``axes`` is
        accepted for API symmetry but currently ignored on CC2.
        """
        self._require_control("home")
        axes_clean = axes.strip().upper()
        if axes_clean not in ("X", "Y", "Z", "XY", "XZ", "YZ", "XYZ", "ALL"):
            raise ValueError(f"axes must be one of X, Y, Z, XY, XZ, YZ, XYZ, or ALL; got {axes!r}")
        # Method 1026 only supports homing all axes in observed firmware.
        result = await self._cc2_request(1026, {}, timeout=CONTROL_TIMEOUT_S)
        return self._wrap_result(1026, result)
    ...
    async def get_file_thumbnail(
        ...
        """Return the embedded PNG thumbnail for a file (method 1045).

        The CC2 stores thumbnails that the slicer embedded in the G-code
        file at upload time. The response contains a base64-encoded PNG.
        Raises :class:`PrinterError` if the file has no thumbnail or the
        printer reports an error.
        """
        result = await self._cc2_request(
            1045,
            {"storage_media": storage, "filename": filename},
        )
        thumbnail_b64 = result.get("thumbnail")
        if not isinstance(thumbnail_b64, str):
            raise PrinterError(f"no thumbnail available for {filename!r}")
        try:
            return base64.b64decode(thumbnail_b64)
        except Exception as err:
            raise PrinterError(f"invalid thumbnail data for {filename!r}") from err
