export default {
  ...
  archives: {
    ...
    no3mfBanner: {
      ...
      bodyInternalStorage: 'Bambu Studio 把切片檔案儲存到了印表機的內部儲存而不是記憶卡上，因此 Bambuddy 透過 FTP 讀不到任何東西。在 H2 系列、P2S 與 X2D 上，「列印」按鈕總是這樣做，只有「傳送」才提供選擇，而它的預設值也是「快取」。這些列印仍會帶著名稱與時間歸檔，只是沒有縮圖與切片中繼資料。要取得完整歸檔，請從 Bambuddy 啟動列印，或改用 OrcaSlicer 切片；若要繼續用 Bambu Studio，請用「傳送」並選擇「外部儲存」，之後再啟動列印。以上都需要印表機中插有記憶卡或 USB 隨身碟。',
  ...
  diagnostic: {
    ...
    check: {
      ...
      external_storage: {
        ...
        warn_internal_storage: '該選項已開啟且儲存媒體在位，但上一次列印仍進入了 Bambuddy 無法讀取的印表機內部儲存。在 H2 系列、P2S 與 X2D 上，無論此選項如何設定，Bambu Studio 的「列印」按鈕總是傳送到那裡。列印會帶著名稱與時間歸檔，但沒有縮圖與切片中繼資料。要取得完整歸檔，請從 Bambuddy 啟動列印、改用 OrcaSlicer 切片，或在 Bambu Studio 中用「傳送」選擇「外部儲存」後再啟動列印。',
