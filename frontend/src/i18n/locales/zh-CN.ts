export default {
  ...
  archives: {
    ...
    no3mfBanner: {
      ...
      bodyInternalStorage: 'Bambu Studio 把切片文件保存到了打印机的内部存储而不是存储卡上，因此 Bambuddy 通过 FTP 读不到任何东西。在 H2 系列、P2S 和 X2D 上，「打印」按钮总是这样做，只有「发送」才提供选择，而它的默认值也是「缓存」。这些打印仍会带着名称和时间归档，只是没有缩略图和切片元数据。要获得完整归档，请从 Bambuddy 启动打印，或改用 OrcaSlicer 切片；若要继续用 Bambu Studio，请用「发送」并选择「外部存储」，之后再启动打印。以上都需要打印机中插有存储卡或 U 盘。',
  ...
  diagnostic: {
    ...
    check: {
      ...
      external_storage: {
        ...
        warn_internal_storage: '该选项已开启且存储介质在位，但上一次打印仍进入了 Bambuddy 无法读取的打印机内部存储。在 H2 系列、P2S 和 X2D 上，无论此选项如何设置，Bambu Studio 的「打印」按钮总是发送到那里。打印会带着名称和时间归档，但没有缩略图和切片元数据。要获得完整归档，请从 Bambuddy 启动打印、改用 OrcaSlicer 切片，或在 Bambu Studio 中用「发送」选择「外部存储」后再启动打印。',
